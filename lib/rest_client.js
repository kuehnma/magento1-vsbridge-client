...
module.exports.RestClient = function (options) {
  ...
  const default_request_token = options.request_token || '';
  ...
  function apiCall(request_data, request_token = '') {
    ...
    request_token = default_request_token || request_token
    logger.debug("Token from options: ", request_token);
    ...
    return new Promise(function (resolve, reject) {
      request({
        url: request_data.url,
        method: request_data.method,
        headers: request_token ? { 'Authorization': 'Basic ' + request_token } : oauth.toHeader(oauth.authorize(request_data, token)),
        json: true,
        body: request_data.body,
